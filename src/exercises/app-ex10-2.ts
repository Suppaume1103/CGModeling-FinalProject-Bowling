...
class ThreeJSContainer {
    ...
    private createScene = () => {
        ...
        // エルミート曲線の接線の導出(立体の向きのため)
        const hermiteTangent = (p0: THREE.Vector3, v0: THREE.Vector3,
            p1: THREE.Vector3, v1: THREE.Vector3, t: number): THREE.Vector3 => {
            const result = p0.clone().multiplyScalar(6 * t * (t - 1))
                .add(v0.clone().multiplyScalar((3 * t - 1) * (t - 1)))
                .add(p1.clone().multiplyScalar(6 * t * (1 - t)))
                .add(v1.clone().multiplyScalar(t * (3 * t - 2)));
            return result;
        }
        ...
        const update: FrameRequestCallback = (_time) => {
            ...
            const tangent = hermiteTangent(points[seg], velocities[seg], points[seg + 1], velocities[seg + 1], t);

            obj.position.copy(pos);
            obj.lookAt(pos.clone().add(tangent));  // 進行方向を向く
